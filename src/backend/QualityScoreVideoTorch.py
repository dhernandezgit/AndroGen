...
class FIDScore(QualityScore):
    ...
    def calculate(self, real_images, generated_images):
        """Compute FID using torchmetrics with proper image formatting."""
        real_images = torch.stack([torch.tensor(img, dtype=torch.uint8).permute(2, 0, 1) for img in real_images]).to(self.device)
        generated_images = torch.stack([torch.tensor(img, dtype=torch.uint8).permute(2, 0, 1) for img in generated_images]).to(self.device)
...
class KIDScore(QualityScore):
    ...
    def __init__(self, feature=2048, subsample_size=1000, device="cuda" if torch.cuda.is_available() else "cpu"):
        self.kid = KernelInceptionDistance(feature=feature, subset_size=subsample_size).to(device)
        self.device = device

    def calculate(self, real_images, generated_images):
        """Compute KID using torchmetrics with proper image formatting."""
        real_images = torch.stack([torch.tensor(img, dtype=torch.uint8).permute(2, 0, 1) for img in real_images]).to(self.device)
        generated_images = torch.stack([torch.tensor(img, dtype=torch.uint8).permute(2, 0, 1) for img in generated_images]).to(self.device)
        ...
        kid_score = self.kid.compute().mean().item()
        self.kid.reset()
        return {"KID": kid_score}

class CLIPFeatureExtractor:
    """Extract features using OpenAI CLIP."""
    def __init__(self, device="cuda" if torch.cuda.is_available() else "cpu"):
        self.device = device
        self.model, self.preprocess = clip.load("ViT-B/32", device=device)

    def extract_features(self, images):
        """Extract feature vectors from images using CLIP."""
        with torch.no_grad():
            images = [self.preprocess(Image.fromarray(img)).unsqueeze(0).to(self.device) for img in images]
            images = torch.cat(images, dim=0)
            features = self.model.encode_image(images).cpu().numpy()
        return features
